...
export default function Comments({ post, user, setPost, reload, setreload }) {
  ...
  function addComment() {
    ...
    fetch("https://uzstragram.onrender.com/addComment", requestOptions)
      .then((res) => res.json())
      .then((data) => {
        setreload(reload + 1);
      });
  ...
  function removeComment(key) {
    ...
    fetch("https://uzstragram.onrender.com/removeComment", requestOptions)
      .then((res) => res.json())
      .then((data) => {
        setreload(reload + 1);
      });
      ...
      {post.comments.map((x) => (
        <div key={x._key}>
          <div className="d-flex  justify-content-between">
            ...
            {x.username === user ? (
              <span
                className="fa fa-trash m-2 fa-2x fa-fade"
                aria-hidden="true"
                onClick={() => removeComment(x._key)}
              >
                {" "}
              </span>
            ) : null}
