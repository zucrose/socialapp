...
export default function Profile({user,setAlert}){
    ...
    function updateProfile(username){
         fetch("/getProfile?user="+username).then((res)=>res.json()).then((data)=>{
            ...
            console.log(data[0]);
            fetch("/getPosts?user="+username).then((res)=>res.json()).then((posts)=>{
    ...
    function followClick(){
        ...
        if(!following){
            ...
            fetch("/addFollower",requestOptions).then((res)=>res.json()).then((data)=>updateProfile(params.username));
        }
        else
        {
            const requestOptions={
            ...
            fetch("/removeFollower",requestOptions).then((res)=>res.json()).then((data)=>updateProfile(params.username));
        }
    ...
    <EditProfile user={user} show={editing} hideCallBack={hideEditCallback} profileData={profileData} setAlert={setAlert}/>
    ...
    <div className="profile-data">
        ...
        <div className="follow-button">
            ...
            {user&&owner?(<Button varaint="primary" onClick={()=>setEditing(true)}>
                Edit
            </Button>)
            :null}
        </div>
