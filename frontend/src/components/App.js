  ...
  return (
    <div className="fill-parent">
      <BrowserRouter>
        <Navbar collapseOnSelect expand="md" bg="dark" variant="dark">
          <Container fluid>
            ...
            <Navbar.Collapse>
              ...
              <Nav>
                   {user ?( 
                    <Navbar.Text>Signed in as:<Link to={"/profile/" + user}>{user}</Link> | {" "}
                    <Button type="button" variant="primary" onClick={()=>{setUser(""); setAlert({variant:"warning" , message:"You are now signed out!"})}}>
                    Logout
                    </Button>
                    </Navbar.Text>
        ...
        {alert?(<AlertDismissible {...alert} deleteAlert={()=>setAlert(null)}></AlertDismissible>):null}
        <Routes>
